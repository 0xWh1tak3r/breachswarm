...
from breachswarm.sdk_context import SDKContext  # Import SDKContext
...
class TestPineconeRetriever(unittest.TestCase):
    ...
    @patch('breachswarm.tools.retriever.pinecone_retrieve.PineconeVectorStore')
    @patch('breachswarm.tools.retriever.pinecone_retrieve.StorageContext')
    @patch('breachswarm.tools.retriever.pinecone_retrieve.VectorStoreIndex')
    def test_create_index(self, VectorStoreIndexMock, StorageContextMock, PineconeVectorStoreMock):
        ...
        self.pinecone_client_mock.create_index.assert_called_once_with(
            name="breachswarm-pinecone", dimension=1536, metric="euclidean", spec=ANY
        )
        self.pinecone_client_mock.Index.assert_called_once_with("breachswarm-pinecone")
    ...
    def test_create_pod_index(self, VectorStoreIndexMock, StorageContextMock, PineconeVectorStoreMock):
        ...
        self.pinecone_client_mock.create_index.assert_called_once_with(
            name="breachswarm-pinecone-pod", dimension=1536, metric="cosine", spec=ANY
        )
        self.pinecone_client_mock.Index.assert_called_once_with("breachswarm-pinecone-pod")
        PineconeVectorStoreMock.assert_called_once()
        StorageContextMock.from_defaults.assert_called_once()
        VectorStoreIndexMock.from_documents.assert_called_once_with(
            ['doc1', 'doc2'],
